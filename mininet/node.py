...
class KindNode ( Host ):
    ...
    def bringIntfUp (self):
        ...
        new_ip = self.params.get('custom_ip',self.ip)
        if self.params.get('custom_subnet'):
            new_ip = new_ip.split('/')[0]
            new_ip += self.params.get('custom_subnet')
        self.cmd('ip address add %s dev %s-eth0' % (new_ip, self.name))
